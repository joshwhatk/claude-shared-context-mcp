...
import { setContext } from '../db/queries.js';
...
export function registerWriteContextTool(): void {
  server.registerTool(
    ...
    async ({ key, content }) => {
      ...
      try {
        // Get existing entry to determine action type
        const { getContext } = await import('../db/queries.js');
        const existing = await getContext(key);
        const action = existing ? 'updated' : 'created';

        // Perform upsert
        const entry = await setContext(key, content);
